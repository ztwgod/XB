...
<html>
	...
	function getResponseResult() {
		...
		var boxCode = x_G("_boxCode").value;
		
		var url = root + "/plat/getGUIResponseText.do?tranNO="+tranNO+"&transType="+transType+"&ssId="+ssId+"&boxCode="+boxCode;
		jQuery.ajax( {
			type : "POST",
			cache : false,
			url : url,
	...
	<body>
		<div style="text-align: center;padding-top: 20px;font-weight: bold;" id="divMsg">
			...
			<input type="hidden" id="_boxCode" name="_boxCode" value="${boxCode}" />
