...
window.onload = function ()
{
	...
	document.oncontextmenu = function (event){
		...
		if(objId.indexOf("box")==-1){
			...
			if(objId.indexOf("Main")!=-1){ //主控制面板，控制整个物箱
				//$("#lastLi").remove();
	...
	function process(value,tagId){
		if(value=="同步"){
			 showTipsWindown();
		}else if(value=="设置收件快递员"){
			 showTipsWindown();
		}else if(value=="设置投件快递员"){
			showTipsWindown();
		}else if(value=="设置维护员"){
			showTipsWindown();
		}else if(value=="物箱状态"){
			showTipsWindown();
		}else if(value=="外围设备状态"){
			showTipsWindown();
		}else if(value == "开箱"){
			showTipsWindown();
		}else if(value == "维修密码发送"){
			showTipsWindown();
			
		}else if(value == "开箱密码重发"){
			showTipsWindown();
		}
	}
};
...
function showTipsWindown(){
	tipsWindown("弹出标题窗口",'iframe:http:\/\/www.baidu.com',"800","600",false,"",1,"","") 
}
